const LOGICAL_VIEWPORT_WIDTH = 240;
const LOGICAL_VIEWPORT_HEIGHT = 320;
const S60_ICON_ASSET_BASE_PATH = "./visuals-to-use/S60v3/";
...
const ICON_NAMES = Object.freeze({
  ...
  FILES: "files.png",
  CONNECTIVITY: "wlan_login.png",
  TOOLS: "folder utilities.png",
  CLOCK: "clock.png",
});
...
const MENU_APPS = Object.freeze([
  ...
  Object.freeze({
    ...
    subtitle: "Network",
    token: "CN",
    iconName: ICON_NAMES.CONNECTIVITY,
  }),
  Object.freeze({
    id: "tools",
    label: "Tools",
...
const TOOLS = Object.freeze([
  Object.freeze({ id: "task-switcher", label: "Task manager", subtitle: "Manage running apps" }),
...
function resolveIconPath(iconName) {
  ...
  return encodeURI(`${S60_ICON_ASSET_BASE_PATH}${iconName}`);
}

function renderIconChip({ iconName, token = "--", label = "", className = "symbian-icon-chip" } = {}) {
  const iconPath = resolveIconPath(iconName);
  const fallbackToken = escapeHtml(token);
  const fallbackClassName = iconPath
    ? "symbian-icon-chip__fallback is-hidden"
    : "symbian-icon-chip__fallback";
  const imageMarkup = iconPath
    ? `<img class="symbian-icon-chip__img" src="${iconPath}" alt="${escapeHtml(
        label ? `${label} icon` : "application icon",
      )}" loading="eager" decoding="async" />`
    : "";

  return `<span class="${escapeHtml(className)}" aria-hidden="true">${imageMarkup}<span class="${fallbackClassName}">${fallbackToken}</span></span>`;
}
...
function isMenuHardwareKey(keyValue) {
  return keyValue === "ContextMenu" || keyValue === "F4" || keyValue === "Menu";
}
...
function createInitialState({ variant }) {
  ...
  return {
    variant,
    brand: "Nokia",
    platformName: "Symbian 9.1 / S60 3rd Edition",
    viewportScale: 1,
    screen: "boot",
    ...
    standbyNotifications: [
      ...
      {
        label: "Calendar",
        subtitle: "Sprint planning 09:30",
        trailing: "today",
        token: "CL",
        iconName: ICON_NAMES.CALENDAR,
      },
...
export function createSymbianShell({ root, variant = "s60-3rd" } = {}) {
  ...
  let menuKeyActive = false;
  let menuKeyLongPressTriggered = false;
  let menuKeyTimeoutId = null;
  ...
  function clearMenuKeyHold({ triggerShortPress = false } = {}) {
    const shouldTriggerShortPress = triggerShortPress && menuKeyActive && !menuKeyLongPressTriggered;

    menuKeyActive = false;
    menuKeyLongPressTriggered = false;

    clearManagedTimeout(menuKeyTimeoutId);
    menuKeyTimeoutId = null;
  ...
  function beginMenuKeyHold() {
    ...
    clearManagedTimeout(menuKeyTimeoutId);
    menuKeyTimeoutId = setManagedTimeout(() => {
      menuKeyTimeoutId = null;

      if (!mounted || !menuKeyActive) {
        return;
      }

      menuKeyLongPressTriggered = true;
      if (state.keylock) {
  ...
  function syncViewportScale({ shouldRender = true } = {}) {
    const rect = root.getBoundingClientRect();
    const availableWidth = Math.max(140, rect.width - 16);
    const availableHeight = Math.max(180, rect.height - 16);
    const rawScale = Math.min(
      availableWidth / LOGICAL_VIEWPORT_WIDTH,
      availableHeight / LOGICAL_VIEWPORT_HEIGHT,
    );
    const nextScale = clamp(rawScale, 0.68, 4);
    const normalizedScale = Number(nextScale.toFixed(4));

    if (normalizedScale === state.viewportScale) {
      return;
    }

    state.viewportScale = normalizedScale;
  ...
  function queueViewportScaleSync() {
    if (resizeRafId != null) {
      return;
    }
  ...
  function handleKeyDown(event) {
    ...
    if (key === "ArrowUp") {
      ...
      handleSoftkey("left");
      handled = true;
    } else if (state.taskSwitcher.open && (loweredKey === "c" || key === "Delete" || key === "Clear")) {
      closeTaskSwitcherSelection();
      handled = true;
    } else if (key === "F6" || key === "Call") {
      handleUtilityCall();
      handled = true;
    } else if (key === "F7" || key === "End" || key === "EndCall") {
      handleUtilityEnd();
      handled = true;
    } else if (key === "Tab") {
      toggleTaskSwitcher();
  ...
  function handleKeyUp(event) {
    ...
    if (!isMenuHardwareKey(event.key)) {
      return;
    }

    const handled = clearMenuKeyHold({ triggerShortPress: true });
  ...
  function handleClick(event) {
    ...
    if (action === "softkey") {
      handleSoftkey(target.dataset.side === "right" ? "right" : "left");
    ...
    return `
      <div class="symbian-statusbar" role="status" aria-live="polite">
        <div class="symbian-statusbar__left">
          ...
          <span class="symbian-statusbar__operator">${escapeHtml(state.carrierName)}</span>
        </div>
  ...
  function renderMenuScreen() {
      ...
      cells.push(`
        <li class="symbian-menu-grid__item${selected}" data-action="menu-index" data-index="${globalIndex}">
          ${renderIconChip({
            ...
            token: app.token,
            label: app.label,
            className: "symbian-menu-grid__icon",
          })}
    ...
    return `
      <main class="symbian-screen symbian-screen--menu">
        ...
        <p class="symbian-status-message">Hold Menu key for Task manager (Tab fallback).</p>
      </main>
    `;
  }
    ...
    return `
      <main class="symbian-screen symbian-screen--standby${state.keylock ? " is-dimmed" : ""}">
        ...
        <section class="symbian-standby__clock-block">
          ...
          <p class="symbian-standby__profile">${state.keylock ? "Keypad locked" : "Press Menu or OK key"}</p>
        </section>
    ...
    return `
      <main class="symbian-screen symbian-screen--boot">
        <section class="symbian-app-body symbian-boot">
          ...
          <p class="symbian-boot__hint">Press OK key to continue</p>
        </section>
      </main>
    `;
  }
      ...
      return `
        <li class="symbian-thumb-grid__item${selected}">
          ${renderIconChip({
            ...
            token: item.token,
            label: item.label,
            className: "symbian-thumb-grid__preview",
          })}
  ...
  function renderSettingsApp() {
    const settings = state.apps.settings;
    const rows = [
      { label: "Profile", value: PROFILES[settings.profileIndex], token: "PF", iconName: ICON_NAMES.SETTINGS },
      { label: "Theme", value: THEMES[settings.themeIndex], token: "TH", iconName: ICON_NAMES.SETTINGS },
      { label: "Network mode", value: NETWORK_MODES[settings.networkIndex], token: "NW", iconName: ICON_NAMES.CONNECTIVITY },
      { label: "Keypad tones", value: KEYPAD_TONES[settings.keypadToneIndex], token: "KT", iconName: ICON_NAMES.SETTINGS },
    ];
  ...
  function renderToolsApp() {
    ...
    return `
      ...
      <section class="symbian-app-body">
        ...
        <p class="symbian-status-message">Run Task manager to jump between running apps.</p>
      </section>
    `;
  }
    ...
    return `
      <div class="symbian-dialog-layer" aria-modal="true" role="dialog">
        <section class="symbian-dialog">
          <h3 class="symbian-dialog__title">Task manager</h3>
  ...
  function render() {
    ...
    root.innerHTML = `
      <div class="symbian-shell" role="application" aria-label="Symbian 9.1 S60 3rd Edition mobile shell">
        <div class="symbian-stage" style="--symbian-scale:${state.viewportScale};">
          <section class="symbian-device">
        ...
        <nav class="symbian-touch-nav" aria-label="Directional navigation">
          ...
          <button type="button" class="symbian-touch-nav__key symbian-touch-nav__key--ok" data-action="select">OK</button>
  ...
  function mount() {
    ...
    root.dataset.runtime = "mobile-symbian-s60-3rd";
    root.tabIndex = -1;

    root.addEventListener("click", handleClick);
    window.addEventListener("keydown", handleKeyDown, true);
    window.addEventListener("keyup", handleKeyUp, true);
    window.addEventListener("resize", queueViewportScaleSync);

    if (typeof window.ResizeObserver === "function") {
      resizeObserver = new window.ResizeObserver(() => {
        queueViewportScaleSync();
      });
      resizeObserver.observe(root);
    }

    syncViewportScale({ shouldRender: false });
    startHeartbeat();
    startBootSequence();
    render();
    setManagedTimeout(queueViewportScaleSync, 0);
  ...
  function unmount() {
    ...
    window.removeEventListener("resize", queueViewportScaleSync);
